...
def run(
    ...
    project=ROOT,  # save results to root directory
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        # Process predictions
        food_detection_results = {item: "No" for item in FOOD_ITEMS}
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    if label in FOOD_ITEMS:
                        food_detection_results[label] = "Yes"
                        with open(OUTPUT_FILE, "a", newline="") as f:
                            writer = csv.writer(f)
                            now = datetime.datetime.now()
                            writer.writerow([now, label])
...
def parse_opt():
    ...
    parser.add_argument("--project", default=ROOT, help="save results to root directory")
